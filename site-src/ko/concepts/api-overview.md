...
*게이트웨이 클래스*는 공통 구성과 동작을 가진 게이트웨이 집합을
정의한다.

*게이트웨이*는 트래픽이 클러스터 내의 서비스로 변환될 수 있는 지점을
요청한다.

*라우트*는 게이트웨이를 통해 들어오는 트래픽이 서비스에 어떻게 매핑되는지 설명한다.

### 게이트웨이 클래스

??? success "v0.5.0 부터 표준 채널"

    `게이트웨이 클래스` 리소스는 GA(정식 출시)되었으며 `v0.5.0` 부터 표준 채널의 일부이다.
    릴리스 채널에 대한 자세한 정보는
...
v1alpha2를 기준으로, API에는 네 가지 라우트 리소스 유형이 포함되어 있다.
구현별로 특정한 사용자 정의 라우트 유형은 다른 프로토콜에 대해 권장된다.
향후 API에 새로운 라우트 유형이 추가될 수 있다.
