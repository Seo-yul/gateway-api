...
이에 대한 자세한 내용은 [트래픽 매칭](/concepts/traffic-matching/) 페이지를 참조하자.
...
## 라우트를 게이트웨이에 연결하기 <a name="attaching-routes-to-gateways"></a>
...
??? example "확장 기능"
    ...
    이 기능의 이름은 `HTTPRouteParentRefPort`이며, 구현체의 GatewayClass(`status.supportedFeatures` 아래) 또는 구현체 비교 페이지(예: [v1.4 비교](/implementations/v1.4.md))에서 구현체가 이를 지원하는지 확인할 수 있다.
...
## 확장 포인트 <a name="extension-points"></a>
