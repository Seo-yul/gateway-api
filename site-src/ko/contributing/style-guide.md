...
## 콜아웃
문서에서 콜아웃을 구현하기 위해 [mkdocs-material admonitions](https://squidfunk.github.io/mkdocs-material/reference/admonitions/)를 사용한다.

### 릴리스 채널
가능한 한, 리소스나 기능이 현재 어떤 릴리스 채널에 포함되어 있는지, 그리고 해당 채널에 도입된 버전이 무엇인지 명확하게 명시하려고 노력한다. 이를 위해 다음과 같은 admonition을 사용한다:

??? success "Standard Channel since v0.5.0"

    `HTTPRoute` 리소스는 GA이며 `v0.5.0`부터 Standard 채널에 포함되었다. 릴리스 채널에 대한 자세한 내용은 [버전 관리 가이드](../concepts/versioning.md)를 참조하자.

??? example "Experimental Channel since v0.3.0"

    `TLSRoute` 리소스는 Alpha이며 `v0.3.0`부터 Experimental 채널에 포함되었다. 릴리스 채널에 대한 자세한 내용은 [버전 관리 가이드](../concepts/versioning.md)를 참조하자.
