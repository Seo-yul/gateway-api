...
Gateway API에 오신 것을 환영한다! 이 페이지는 새로운 사용자이든 다른 인그레스 솔루션에서 마이그레이션하든 시작하기에 가장 좋은 곳이다.
...
Gateway API를 지원하는 [여러 프로젝트](../../implementations.md)가 있다.
...
!!! danger "이전 실험적 채널 릴리스에서 업그레이드"
    ...
    [v1.1 업그레이드 참고 사항](#v11-upgrade-notes)을 참조하자.
...
GatewayClass, Gateway, HTTPRoute, ReferenceGrant가 포함된다.
이 채널을 설치하려면 다음 kubectl 명령을 실행하자:

```bash
kubectl apply --server-side -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.5.0/standard-install.yaml
```

이 kubectl 명령 옵션에 대해 더 알아보려면 [서버 사이드 적용 문서](https://kubernetes.io/docs/reference/using-api/server-side-apply/)를 참조하자.
...
[버전 관리 문서](../../concepts/versioning.md)를 참조하자.

실험적 채널을 설치하려면 다음 kubectl 명령을 실행하자:
...
API의 `v1alpha3`를 지원하도록 업데이트될 때까지 기다리자. `v1alpha3`를
