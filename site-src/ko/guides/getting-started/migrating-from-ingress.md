...
!!! note "Ingress-NGINX 사용자이신가요?"
    Ingress-NGINX를 사용하고 있다면, 더 맞춤화된 조언과 리소스를 위해 [Ingress-NGINX 마이그레이션 가이드](./migrating-from-ingress-nginx.md)도 확인해 보자.

이 가이드는 변환을 도와준다. 다음 내용을 다룬다.
...
*   *정책.* 게이트웨이 구현은 인증과 같은 데이터 플레인 기능을 노출하기
    ...
    규정한다. 자세한 내용은 [정책 연결 가이드](../../reference/policy-attachment.md)를
    참조하자. 위의 *외부 참조*와 달리, Gateway API 리소스는 정책을
...
Gateway API에서는 진입점을
[Gateway](../../api-types/gateway.md) 리소스에 명시적으로 정의해야 한다.
...
> [HTTPRoute 문서](../../../reference/spec.md#httproutespec)를 참조하자.
...
*   게이트웨이 구현은 리스너에 연결된 모든 HTTPRoute의 라우팅 규칙을
    병합해야 한다.
*   충돌은
    [API 설계 가이드: 충돌](../api-design.md#conflicts)에 규정된 대로 처리해야 한다.
...
[확장 지점](#extensibility-approach)을 사용할지 확인하자.
