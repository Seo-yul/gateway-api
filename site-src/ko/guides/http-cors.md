...
???+ info "확장 지원 기능: HTTPRouteCORS"
    이 기능은 확장 지원의 일부이며, 구현체가 `HTTPRouteCORS` 기능을 지원해야 한다. 지원 수준에 대한 자세한 내용은 [적합성 가이드](../concepts/conformance.md)를 참고한다.

??? success "v1.5.0부터 표준 채널"
    `HTTPRouteCORS` 기능은 `v1.5.0`부터 표준 채널의 일부이다. 릴리스 채널에 대한
    자세한 내용은 [버전 관리 가이드](../concepts/versioning.md)를 참고한다.

[**HTTPRoute(HTTP 라우트)** 리소스](../api-types/httproute.md)를 사용하여
