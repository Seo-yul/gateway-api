...
Gateway API는 크로스 **네임스페이스(네임스페이스)** 라우팅을 핵심 기능으로 지원한다.
...
**클러스터(클러스터)** 전체 라우팅 구성의 다양한 부분에 대한 접근과 제어를 효과적으로
...
제한하는 게이트웨이 **리스너(리스너)**의 필드이다. 게이트웨이는 네임스페이스와
...
이는 관리 오버헤드를 줄이는 효과적인 **정책(정책)**이다. 앱 소유자는 앱이
...
[라우팅 우선순위](../../reference/spec.md#httprouterule)는 가장 구체적인 매치에 의해
결정되며, 충돌은 [충돌 해결](api-design.md#conflicts)에 따라 처리된다.
